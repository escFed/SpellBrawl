...
public class CharacterGrab : MonoBehaviour
{
    ...
    public ICharacterState ResolveGrabState()
    {
        ...
        bool canPivotGrab = controller.stateMachine.CurrentState == controller.States.Move;

        Input?.ConsumeGrab();

        if (canPivotGrab && hasHorizontal && inputOppositeFacing)
            return controller.States.PivotGrab;

        return controller.States.Grab;
    }
